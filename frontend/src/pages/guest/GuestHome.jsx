...
import { Compass, UtensilsCrossed, MessageSquare, Wifi, Clock, Calendar, MapPin, Coffee, User, ClipboardCheck, ChevronRight } from "lucide-react";
import GuestLayout from "@/components/GuestLayout";
import api, { getSettings, seedData } from "@/lib/api";
import { getPropertyId, getPropertySlug } from "@/lib/store";
  ...
  return (
    ...
    >
      <div className={`flip-card-inner relative w-full h-full transition-transform duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
        ...
        <div className="flip-card-front absolute w-full h-full backface-hidden rounded-xl overflow-hidden shadow-md">
          <div className="relative h-full">
            {item.image_url ? (
              ...
              <div className={`w-full h-full ${type === 'food' ? 'bg-gradient-to-br from-[#FFC107] to-[#F9A825]' : 'bg-gradient-to-br from-[#0D7377] to-[#14919B]'}`} />
            )}
        ...
        {/* Back */}
        <div className={`flip-card-back absolute w-full h-full backface-hidden rounded-xl overflow-hidden shadow-md rotate-y-180 ${type === 'food' ? 'bg-gradient-to-br from-[#FFC107] to-[#F9A825]' : 'bg-gradient-to-br from-[#0D7377] to-[#14919B]'}`}>
...
export default function GuestHome() {
  ...
  const modules = [
    ...
    },
    {
      ...
      path: "/food",
      color: "from-[#FFC107]/80 to-[#F9A825]/90",
    },
    {
      id: "requests",
      title: "Requests",
      ...
      <div className="min-h-screen" data-testid="guest-home">
        ...
        <div className="relative h-[38vh] min-h-[300px] overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-12">
            ...
            <p className="font-accent text-[#FFC107] text-lg md:text-xl mb-1 animate-fade-in">
        ...
        {propertyInfo && (
          <div className="px-4 md:px-8 -mt-6 relative z-10 mb-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <Card className="bg-white/90 backdrop-blur-sm border-none shadow-md">
                <CardContent className="p-3 text-center">
                  <Clock className="w-5 h-5 mx-auto text-[#F9A825] mb-1" />
              ...
              {propertyInfo.wifi_name && (
                <>
                  <Card className="bg-white/90 backdrop-blur-sm border-none shadow-md">
                    <CardContent className="p-3 text-center">
                      <Wifi className="w-5 h-5 mx-auto text-[#FFC107] mb-1" />
          ...
          <div className="px-4 md:px-8 mb-6">
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {propertyInfo.today_events.map((event, i) => (
                <Card key={i} className="min-w-[200px] bg-gradient-to-br from-[#0D7377]/10 to-[#FFC107]/10 border-none">
        ...
        {propertyInfo?.food_to_try?.length > 0 && (
          <div className="px-4 md:px-8 pb-8">
            <h2 className="text-xl text-[#F9A825] mb-4 flex items-center gap-2 font-bold" style={{fontFamily: "'Quicksand', sans-serif"}}>
              <Coffee className="w-5 h-5 text-[#FFC107]" /> Where to Eat & What to Try
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
