from fastapi import FastAPI, APIRouter, HTTPException, Query, Form, Request, Depends, Header
...
JWT_SECRET = os.environ.get('JWT_SECRET', 'change-this-in-production-use-strong-secret')
JWT_EXPIRY_HOURS = 4

def create_jwt_token(user_id: str, username: str, role: str, property_id: str = None) -> str:
...
async def verify_webhook_secret(x_webhook_secret: str = Header(None, alias="X-Webhook-Secret")):
...
async def create_or_update_property_info(info: PropertyInfoCreate, current_user: dict = Depends(require_admin_or_manager)):
...
async def create_experience(experience: ExperienceCreate, current_user: dict = Depends(require_admin_or_manager)):
...
async def update_experience(experience_id: str, experience: ExperienceCreate, current_user: dict = Depends(require_admin_or_manager)):
...
async def delete_experience(experience_id: str, current_user: dict = Depends(require_admin_or_manager)):
...
async def create_menu_item(item: MenuItemCreate, current_user: dict = Depends(require_admin_or_manager)):
...
async def update_menu_item(item_id: str, item: MenuItemCreate, current_user: dict = Depends(require_admin_or_manager)):
...
async def delete_menu_item(item_id: str, current_user: dict = Depends(require_admin_or_manager)):
...
async def save_settings(settings: SettingsCreate, current_user: dict = Depends(require_admin_or_manager)):
...
async def checkin_webhook(request: Request, _: bool = Depends(verify_webhook_secret)):
    """
